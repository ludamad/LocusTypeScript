...
namespace ts {
    ...
    export function createTypeChecker(host: TypeCheckerHost, produceDiagnostics: boolean): TypeChecker {
        ...
        function checkResolvedBlockScopedVariable(result: Symbol, errorLocation: Node): void {
            ...
            let isUsedBeforeDeclaration:boolean = !isDefinedBefore(declaration, errorLocation);
        ...
        function createConcreteTypeIfCheckable(target: Type) {
        ...
        function unconcrete(target: Type) {
            Debug.assert(target != null);
...
/*                        let declareTypeDecl = <DeclareTypeDeclaration>getSymbolDecl(type.symbol, SyntaxKind.DeclareType);
 ...
 //                       }
        ...
        // [ConcreteTypeScript]
        function isCementedDeclaration(declaration: Declaration): boolean {
            if (declaration.parent && declaration.parent.kind === SyntaxKind.SourceFile) {
                // Always cement global declarations:
                return true;
            }
            if (declaration.flags & NodeFlags.Export) {
        ...
        function resolveClassOrInterfaceMembers(type: InterfaceType): void {
            ...
            if (baseTypes.length) {
                ...
                for (let baseType of baseTypes) {
                    // [ConcreteTypeScript]
                    baseType = <ObjectType> unconcrete(baseType);
                    // [/ConcreteTypeScript]
...
//                        throw new Error("TODO figure out inheritance");
        ...
        function resolveIntermediateFlowType(type: IntermediateFlowType): void {
            ...
            for (let t of type.flowData.flowTypes.map(({type}) => type)) {
        ...
        function resolveTypeReferenceMembers(type: TypeReference): void {
            ...
            forEach(getBaseTypes(target), baseType => {
                // [ConcreteTypeScript] 
                let instantiatedBaseType = <ObjectType>unconcrete(instantiateType(baseType, mapper));
                // [/ConcreteTypeScript] 
        ...
        function resolveStructuredTypeMembers(type: ObjectType): ResolvedType {
            if (!(<ResolvedType>type).members) {
                ...
                if (type.flags & (TypeFlags.Class | TypeFlags.Interface | TypeFlags.Declare)) {
                    ...
                    resolveIntermediateFlowType(<IntermediateFlowType> type);
                }
                // [/ConcreteTypeScript]
                else if (type.flags & TypeFlags.Anonymous) {
        ...
        function getPropertiesOfUnionOrIntersectionType(_type: UnionOrIntersectionType): Symbol[] {
            ...
            // concrete types are the properties of their non-concrete
            // equivalent
            let type = <UnionOrIntersectionType> unconcrete(_type);
            // [/ConcreteTypeScript]
        ...
        function getPropertiesOfIntermediateFlowType(type: IntermediateFlowType): Symbol[] {
            for (let current of type.flowData.flowTypes.map(({type}) => type)) {
                for (let prop of getPropertiesOfType(current)) {
                    getPropertyOfIntermediateFlowType(type, prop.name);
                }
            }
            for (let memberName of Object.keys(type.flowData.memberSet)) {
        ...
        function getPropertiesOfType(type: Type): Symbol[] {
            ...
            if (type.flags & TypeFlags.IntermediateFlow) {
                return getPropertiesOfIntermediateFlowType(<IntermediateFlowType>type) 
            }
        ...
        function getPropertyOfIntermediateFlowType(type: IntermediateFlowType, name: string): Symbol {
        ...
        function getPropertyOfUnionOrIntersectionType(type: UnionOrIntersectionType, name: string): Symbol {
            ...
            // concrete types are the properties of their non-concrete
            // equivalent
            type = <UnionOrIntersectionType> unconcrete(type);
            // [/ConcreteTypeScript]
            let properties = type.resolvedProperties || (type.resolvedProperties = {});
            if (hasProperty(properties, name)) {
                return properties[name];
        ...
        function getTypeFromBecomesTypeNode(node: BecomesTypeNode): Type {
            let links = getNodeLinks(node);
            if (!links.resolvedType) {
                let startingType:Type = emptyObjectType;
        ...
        function checkTypeRelatedTo(
            ...
            function isRelatedTo(source: Type, target: Type, reportErrors?: boolean, headMessage?: DiagnosticMessage): Ternary {
                ...
                if (source.flags & TypeFlags.TypeParameter) {
                    ...
                    if (unconcrete(apparentType).flags & (TypeFlags.ObjectType | TypeFlags.Intersection) && unconcrete(target).flags & TypeFlags.ObjectType) {
            ...
            function isIdenticalTo(source: Type, target: Type): Ternary {
                // [ConcreteTypeScript]
                if (isConcreteType(source) && isConcreteType(target)) {
                    let result = isTypeIdenticalTo(unconcrete(source), unconcrete(target));
...
*/
        ...
        function getReturnTypeFromBody(func: FunctionLikeDeclaration, contextualMapper?: TypeMapper): Type {
            let type = getReturnTypeFromBodyWorker(func, contextualMapper); 
            return getBindingType(type);
        }
        ...
        function checkInExpression(node: BinaryExpression, leftType: Type, rightType: Type): Type {
            ...
            // The result is always of the Boolean primitive type.
            if (!isTypeAnyOrAllConstituentTypesHaveKind(/*ConcreteTypeScript*/ unconcrete(leftType), TypeFlags.StringLike | TypeFlags.NumberLike | TypeFlags.ESSymbol)) {
                error(node.left, Diagnostics.The_left_hand_side_of_an_in_expression_must_be_of_type_any_string_number_or_symbol);
            }
            if (!isTypeAnyOrAllConstituentTypesHaveKind(/*ConcreteTypeScript*/ unconcrete(rightType), TypeFlags.ObjectType | TypeFlags.TypeParameter)) {
        ...
        function expectBrandIdentifierGetType(node: Node):Type {
            ...
            let ident = <Identifier> node;
            let brandSymbol = resolveName(ident.parent, ident.text, SymbolFlags.Type, Diagnostics.ConcreteTypeScript_Expected_identifier_which_resolves_to_type_created_with_declare, ident);
        ...
        function checkBinaryExpression(node: BinaryExpression, contextualMapper?: TypeMapper) {
            ...
            switch (operator) {
                ...
                case SyntaxKind.PlusEqualsToken:
                    ...
                    if (leftType.flags & (TypeFlags.Undefined | TypeFlags.Null)) leftType = unconcrete(rightType);
                    if (rightType.flags & (TypeFlags.Undefined | TypeFlags.Null)) rightType = unconcrete(leftType);
                    // [/ConcreteTypeScript]
        ...
        function getMinimalTypeList(types: Type[]) {
            ...
            function isRedundant(i) {
                for (let j = 0; j < types.length; j++) {
                    ...
                    let cj = isConcreteType(types[j]), ci = isConcreteType(types[i]);
                    let tj = unconcrete(types[j]), ti = unconcrete(types[i]);
                    if (isTypeIdenticalTo(tj, ti)) {
                        if (+cj > +ci) { // Concreteness of j is stronger?
                    ...
                    // If we are a subtype of any type, don't include:
                        ...
                        if (+cj >= +ci) { // Concreteness of j at least ast strong?
                            return true; // Obsoleted by stronger concrete version
                        }
                    }
                }
...
//                return flowDataFormalType(currentFlowData);
       ...
       ***********************************************************************************************************/
        ...
        function flowUnionTypesDegradeToMembers(flowTypesA:FlowType[], flowTypesB:FlowType[]) : FlowData {
            let hasTypeInA = ({type}) => flowHasType(flowTypesA, type);
            let commonTypes = flowTypesB.filter(hasTypeInA);
            let hasTypeInCommon = ({type}) => flowHasType(commonTypes, type);
            let differentTypes = flowTypesA.concat(flowTypesB).filter(hasTypeInCommon);

            let doesntHavePropertyAlready = ({name}:Symbol) => {
            ...
            let memberSet:FlowMemberSet = {};
            for (let {type, firstBindingSite} of differentTypes) {
                for (let prop of getPropertiesOfType(type).filter(doesntHavePropertyAlready)) {
                    let propType:Type = getTypeOfSymbol(prop);
                    // Add a 'weak member' (non-concrete, not definitely assigned)
                    memberSet[prop.name] = {
                        key: prop.name, 
                        definitelyAssigned: false, 
                        conditionalBarrierPassed: true, 
                        flowTypes: [{type: unconcrete(propType), firstBindingSite}]
                    };
                }
            }
            return {flowTypes: commonTypes, memberSet}
        }
        ...
        function flowDataGetPropertyType({memberSet, flowTypes}:FlowData, property:string): Type{
            ...
            for (let {type} of flowTypes) {
                let typeProp = getPropertyOfType(type, property);
                if (typeProp) {
        ...
        function flowDataBecomesType(contextNode:Node, flowData:FlowData, targetType:Type): FlowData {
            ...
            let startingType = flowDataFormalType(flowData);
        ...
        function flowDataPopConditionalMarks(oldData:FlowData, newData:FlowData) : FlowData {
            return {
        ...
        function flowDataAssignment(data:FlowData, key: string, flowType: FlowType): FlowData {
            return {
                flowTypes: data.flowTypes, 
                memberSet: flowAssignment(data.memberSet, key, flowType)
            };
        }
        function flowDataConditionalBarrier(data:FlowData, conditionalBarrierPassed = true) : FlowData {
            return {
                flowTypes: data.flowTypes, 
        ...
        function canHaveFlowData(obj:Node) {
        ...
        function getScopeContainerWorker(obj:Node) {
            ...
            if (obj.kind === SyntaxKind.ThisKeyword || (<Identifier>obj).text === "this") {
...
//        }
        ...
        function computeFlowDataForNonLocusTypeWorker(scope: Node, isReference: ReferenceDecider, initialFlowData: FlowData, targetType: Type): FlowData {
            ...
            return computeFlowDataWorker(
                ...
                /*Non-inferred member types (besides base types): */ (member) => {
                    let property = getPropertyOfType(targetType, member);
        ...
        function computeFlowDataWorker(// Refer to the same object throughout a recursion instance:
            ...
            function scanConditionalExpression(node: ConditionalExpression) {
                ...
                let whenTrue = recurse(node.whenTrue, flowDataConditionalBarrier(prev));
                let whenFalse = recurse(node.whenFalse, flowDataConditionalBarrier(prev));
    ...
    //                prev = bodyScan.merge(prev);
            ...
            function scanTryStatement(node: TryStatement) {
                ...
                let ifCatch = recurse(node.catchClause, flowDataConditionalBarrier(flowDataUnion(ifTry, prev)));
